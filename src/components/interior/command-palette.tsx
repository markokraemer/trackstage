  ...
  const surface = (
    <div
      className={`overflow-hidden rounded-xl border border-border bg-card ${
        overlaid
          ? "w-full max-w-[520px] shadow-[0_1px_2px_rgba(27,30,39,0.07),0_28px_56px_-24px_rgba(27,30,39,0.5)]"
          : ""
      } ${className}`}
    >
      <div className="flex h-11 items-center gap-2.5 border-b border-border px-3">
        ...
        />
        <span className="min-w-[3ch] shrink-0 text-right font-mono text-[9.5px] text-muted-foreground tabular-nums">
          {count}
        </span>
      </div>
      <div className="relative" style={{ height }}>
        <ul
          ...
          className="absolute inset-0 flex [scrollbar-gutter:stable] flex-col gap-[2px] overflow-y-auto overscroll-contain p-[5px]"
        >
          {results.map((item) => {
                  ...
                  {item.shortcut ? (
                    <span className="ml-auto flex shrink-0 items-center gap-1">
                      {item.shortcut.map((key) => (
                        <span
                          key={key}
                          className="flex h-[18px] min-w-[18px] items-center justify-center rounded-[5px] border border-border px-1 font-mono text-[9.5px] text-muted-foreground tabular-nums"
                        >
                          {key}
                        </span>
                      ))}
