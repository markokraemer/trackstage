  ...
  return (
    <div className={`w-full select-none ${className}`}>
      <div className="mb-2.5 flex items-baseline justify-between gap-3">
        <span id={labelId} className="text-[12.5px] text-muted-foreground">
          {label}
        </span>
        <span className="grid justify-items-start">
          <span
            aria-hidden
            className="invisible col-start-1 row-start-1 font-mono text-[11px] whitespace-pre tabular-nums"
          >
            {widest}
          </span>
          <span
            aria-hidden
            className="col-start-1 row-start-1 font-mono text-[11px] whitespace-pre text-foreground tabular-nums"
          >
            {format(value)}
            <motion.span
              initial={false}
      ...
      <div
        ref={trackRef}
        {...trackProps}
        className={`relative h-9 w-full rounded-md outline-none focus-visible:bg-primary/[0.06] focus-visible:shadow-[inset_0_0_0_1px_var(--primary)] ${
          disabled
      ...
      >
        <div className="pointer-events-none absolute inset-x-0 top-[9px] h-[10px] overflow-hidden rounded-[5px] bg-border">
          <div
