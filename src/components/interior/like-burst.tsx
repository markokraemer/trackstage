  ...
  return (
    <span className={`inline-flex items-center ${className}`}>
      <button
        ...
        className="inline-flex h-9 items-center gap-2 rounded-md border border-border bg-card px-3 text-[13px] font-medium text-foreground outline-none select-none focus-visible:ring-2 focus-visible:ring-ring disabled:opacity-50"
      >
          ...
          {!reduced && burst > 0 ? (
            ...
            >
              {SPARKS.map((spark, i) => (
                <motion.span
                  key={i}
                  className="absolute block rounded-[1.5px] bg-foreground"
                  style={{
                    width: spark.size,
                    height: spark.size,
        ...
        <span
          aria-hidden
          className="grid overflow-hidden text-[12px] text-muted-foreground tabular-nums"
        >
