          ...
          return (
            ...
            >
              <span className="relative grid size-4 shrink-0 place-items-center">
                <AnimatePresence initial={false}>
                  {row.status === "done" ? (
                    <motion.span
                      key="done"
                      className="col-start-1 row-start-1 grid size-4 place-items-center rounded-[5px] bg-status-green-dot/[0.14] text-status-green-fg"
                      initial={
                    ...
                    <motion.span
                      key="error"
                      className="col-start-1 row-start-1 grid size-4 place-items-center rounded-[5px] bg-destructive/[0.12] text-destructive"
                      initial={
                        reduced ? { opacity: 0 } : { opacity: 0, scale: 0.4 }
                      }
                      animate={{ opacity: 1, scale: 1 }}
                    ...
                    <motion.span
                      key="pending"
                      className="col-start-1 row-start-1 size-[5px] rounded-[2px] bg-input"
                      initial={{ opacity: 0 }}
                      animate={{ opacity: 1 }}
                      exit={{ opacity: 0, transition: STILL }}
                      transition={STILL}
              ...
              {row.status === "active" && !reduced ? (
                <motion.span
                  className="min-w-0 flex-1 truncate bg-[linear-gradient(90deg,#64748B_38%,#1B1E27_50%,#64748B_62%)] [background-size:220%_100%] bg-clip-text text-[12.5px] font-medium text-transparent"
