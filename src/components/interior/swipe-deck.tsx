...
function DeckCard({
  ...
  const badge = (side: -1 | 1, text: string, place: string) => {
    ...
    return (
      <motion.span
        ...
        className={`pointer-events-none absolute top-3 rounded-[6px] border bg-card px-2 py-1 text-[10.5px] font-semibold tracking-[0.08em] whitespace-nowrap uppercase transition-colors duration-150 ${
  ...
  return (
    <motion.div
      ...
      className={`absolute inset-x-5 top-0 overflow-hidden rounded-xl border border-border bg-card select-none ${
        active
          ? "cursor-grab shadow-[0_1px_2px_rgba(27,30,39,0.06),0_16px_32px_-18px_rgba(27,30,39,0.55)] active:cursor-grabbing"
          : "shadow-[0_1px_2px_rgba(27,30,39,0.05),0_6px_14px_-12px_rgba(27,30,39,0.4)]"
      }`}
    >
      {children}
...
export function SwipeDeck<T>({
  ...
  const control =
    "inline-flex h-8 items-center gap-1.5 rounded-md border border-border bg-card px-2.5 text-[12px] font-medium text-foreground outline-none transition-[background-color,border-color,opacity] duration-150 hover:bg-muted focus-visible:border-ring"
  ...
  return (
    <div className={`w-full ${className}`}>
      <div
        ...
        className="relative w-full overflow-hidden rounded-xl outline-none focus-visible:shadow-[0_0_0_1px_var(--primary)]"
        {...deck.deckProps}
      >
        <div className="absolute inset-0 overflow-hidden [mask-image:linear-gradient(to_right,transparent,black_20px,black_calc(100%-20px),transparent)]">
          <motion.div
            ...
            style={{ height }}
            className="absolute inset-x-5 top-0 z-0 grid place-items-center rounded-xl bg-muted/70 px-4 text-center text-[12.5px] text-muted-foreground shadow-[inset_0_1px_2px_rgba(27,30,39,0.07)]"
          >
            {emptyLabel}
          </motion.div>
      ...
      <div className="mt-3 grid h-8 grid-cols-[1fr_auto_1fr] items-center gap-3">
        ...
        <span className="flex items-center gap-2 font-mono text-[10.5px] text-muted-foreground tabular-nums">
          ...
          <button
            ...
            onClick={deck.undo}
            inert={!deck.canUndo}
            className={`inline-flex items-center gap-1 rounded-[5px] px-1 py-0.5 text-foreground transition-[background-color,box-shadow,opacity] duration-150 outline-none hover:bg-muted focus-visible:bg-primary/[0.06] focus-visible:shadow-[inset_0_0_0_1px_var(--primary)] ${spent(
              !deck.canUndo
            )}`}
          >
            {ICON_UNDO}
