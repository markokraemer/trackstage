...
const TILE = "absolute left-0 top-0 select-none rounded-lg bg-border p-[3px]"
const WELL =
  "relative grid size-full place-items-center overflow-hidden rounded-[7px] bg-muted font-medium leading-none text-muted-foreground"
...
export function PresenceAvatars({
  ...
  const chipClass =
    "absolute left-0 top-0 grid place-items-center rounded-md border border-border bg-card font-mono text-[10.5px] leading-none tabular-nums text-muted-foreground outline-none ring-2 ring-card"
          ...
          {overflow > 0 &&
            (onOverflowSelect ? (
              <motion.button
                ...
                className={`${chipClass} focus-visible:border-ring`}
                {...chipMotion}
              >
