...
export function PressDepth({
  ...
  return (
    <button
      ...
      className="group relative inline-flex rounded-md align-middle outline-none select-none disabled:opacity-50"
      {...bind}
    >
      <span
        aria-hidden
        style={{ top: depth }}
        className="absolute inset-x-0 bottom-0 rounded-md bg-input"
      />
      <motion.span
        initial={false}
        animate={{
        ...
        className={`relative inline-flex h-9 items-center justify-center gap-2 rounded-md border border-border bg-card px-3.5 text-[13px] font-medium text-foreground group-focus-visible:ring-2 group-focus-visible:ring-ring ${className}`}
      >
        <motion.span
          aria-hidden
          initial={false}
          ...
          transition={reduced ? { duration: 0 } : PRESS}
          className="pointer-events-none absolute inset-0 rounded-md shadow-[inset_0_1.5px_0_rgba(255,255,255,0.95),inset_0_-1px_0_rgba(27,30,39,0.06)]"
        />
        {children}
      </motion.span>
    </button>
