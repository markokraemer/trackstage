...
const TONES = {
  none: { bar: "bg-input", text: "text-muted-foreground" },
  danger: { bar: "bg-destructive", text: "text-destructive" },
  caution: { bar: "bg-status-amber-dot", text: "text-status-amber-fg" },
  safe: { bar: "bg-status-green-dot", text: "text-status-green-fg" },
} as const
    ...
    <div className={`w-full ${className}`}>
      ...
      >
        {Array.from({ length: max }, (_, i) => (
          <div
            key={i}
            className="relative h-1.5 overflow-hidden rounded-[2px] bg-border"
          >
            <motion.span
      ...
      <div className="mt-2 flex h-5 items-center justify-between gap-3">
        <span className="inline-grid text-[12.5px] leading-5 font-medium">
          {labels.map((text, i) => (
            <motion.span
              key={text}
              aria-hidden
        ...
        <motion.span
          aria-hidden
          className="text-[11.5px] leading-5 whitespace-nowrap text-status-amber-fg"
          initial={false}
          ...
          {evaluated.map((rule) => (
            <li key={rule.id} className="flex items-center gap-2">
              <span className="relative grid size-[14px] shrink-0 place-items-center rounded-[4px] border border-border text-primary-foreground">
                <motion.span
                  className="absolute inset-0 rounded-[3px] bg-status-green-dot"
                  initial={false}
