...
export const Route = createFileRoute("/app/$workspaceSlug/$eventSlug/forms/$formId")({
  ...
  if (isError) {
    return (
      <EmptyState
        ...
        action={
          <Link
            to={eventRef ? appLink.forms(eventRef) : legacyAppLink.forms}
...
function FormEditor({
  ...
  /** The form doc plus its event's slug and workspace slug — see `api.forms.get`. */
  form: Doc<"forms"> & { eventSlug: string; workspaceSlug: string }
  trackNames: Array<string>
  timezone?: string
  eventRef: { workspaceSlug: string; eventSlug: string } | undefined
}) {
