...
/** Accepted sessions, paginated — GET /v1/event/{slug}/sessions. */
export const apiSessionsPage = internalQuery({
  args: { slug: v.string(), page: v.number(), pageSize: v.number() },
  handler: async (ctx, args) => {
    const program = await loadProgram(ctx, args.slug)
    if (!program) return null
    const items = program.sessions.map((session) => ({
      id: session._id,
      title: session.title,
      description: session.description ?? null,
      status: "accepted",
      startTime: session.startsAt ?? null,
      endTime: session.endsAt ?? null,
      durationMinutes: session.durationMinutes ?? null,
      location: session.room?.name ?? null,
...
/** Accepted speakers, paginated — GET /v1/event/{slug}/speakers. */
export const apiSpeakersPage = internalQuery({
  args: { slug: v.string(), page: v.number(), pageSize: v.number() },
  handler: async (ctx, args) => {
    const program = await loadProgram(ctx, args.slug)
    if (!program) return null
    const byId = new Map(program.sessions.map((s) => [s._id, s]))
    const speakerDocs: Array<Doc<"people"> | null> = []
    for (const speaker of program.speakers) {
      speakerDocs.push(await ctx.db.get(speaker._id))
    }

    const items = program.speakers
      .map((speaker, index) => {
        const doc = speakerDocs[index]
        const mine = (program.sessionsByPerson.get(speaker._id) ?? [])
          .map((id) => byId.get(id))
          .filter((s): s is PublicSession => Boolean(s))
        return {
          id: speaker._id,
          ...
          firstName: speaker.firstName,
          lastName: speaker.lastName,
          // The API mirrors Sessionboard's, which exposes speaker email to
          // Bearer-token holders. The browser-facing queries never do.
          ...
          sessions: mine.map((s) => ({
            id: s._id,
            title: s.title,
            startTime: s.startsAt ?? null,
            endTime: s.endsAt ?? null,
            location: s.room?.name ?? null,
          })),
        }
      })
      .sort(
