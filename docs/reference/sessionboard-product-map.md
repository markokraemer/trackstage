...
- The **Add Field** modal is exactly five controls (frame-verified): **Field Name\*** · **Field Type\*** (default `Text`) · **Field Description** ⓘ (placeholder *"Description of field…"*, internal only) · **Maximum Length** (placeholder **"Default character limit: 255"**) · **Field Level** (default **`Event Field`**) → Save / Cancel. The **Select fields** picker beside it lists every existing field with **its type rendered underneath the name** (`Capacity · Number`, `Display Session · Checkbox`, `Ends At · Datetime`, `Evaluator Feedback · Textarea`), has `Select All / Remove All`, an `All Fields | Global | Event` segmented filter, a **Selected Fields** basket, and `Cancel · Done · ⊕ Create New Field`. `NEW`
- **Default-off is the shipped state.** On a fresh form, only Title and Description are on; **Format, Tags, Track and Level all ship toggled OFF**, and on the participant page **Biography and Headshot ship OFF** too. `NEW` — worth matching: their default CFP is deliberately tiny.
- The field library contains a standard **`Display Session` (Checkbox)** session field — a per-session public-visibility flag, and the session-level counterpart to the per-speaker eye icon. `NEW`, and it is the missing half of sbek **CNT-12**: their answer to "keep this out of public output" is a boolean on the record, not a separate approval workflow.
- In this build the participant step is labelled **"Contact Information"** with the description *"Collect information for the account's primary contact / login user."* — i.e. page 2 collects the **submitter/login identity**, and page 4 collects the speakers. `CONFIRMS` the Session Submitter ≠ Speaker distinction, at the form level.
...
- **Verbatim helper copy and defaults** (frame-verified, `video-session-submission-form`) — worth copying almost word for word, since it is what non-technical organizers have been trained on: `NEW`
  - Speaker Information page description: *"Give us information about yourself and your credentials for presenting at our event."*
  - *Allow users to submit new information for existing contacts* — "If enabled, users will be able to submit new information for existing contacts. If disabled, existing contacts will need to login to the portal to submit any additional information you want to collect."
  - *Notify existing contacts that they have been added to a submission* — "If enabled, existing contacts will receive an email notification to login to the portal to view and update their information. If disabled, existing contacts will not receive any notification upon submission."
  - *Close Date* — "If set, form and submissions will close after specified date", input mask `MM/DD/YYYY @ hh:mm a`.
  - *Send Reminder Email* — "Send an email reminder to submitters who have saved a draft but not completed the form five days and one day before the close date."
  - *Automatically redirect to the user's portal after 10 seconds* — "If enabled, we will automatically redirect the user from the confirmation page to the user's portal after 10 seconds. If disabled, they will need to click the 'Continue to portal' button."
  - Default **confirmation email** body: *"Thank you for submitting your session. We will be in touch regarding its status soon. / To access your speaker portal, click the button below to log in with your email address. If you already have a Sessionboard password, use it to sign in. Otherwise, you'll be prompted to set one. Or, if your organization uses Single Sign-On (SSO), you can sign in with those credentials."*
  - Default **success page**: *"Thank you for submitting to present at our event! / You will receive a confirmation email shortly with a link to your speaker portal. We will review sessions over the next few weeks and then notify you regarding your status. / Next, you will be logged into your speaker portal where you can see if there are any tasks to complete. / If you would like to submit another session, please **click here** to return to the submission form."* — note the **"submit another session" link**, which is how they handle multi-submission without a dedicated flow.
  - Speaker Information default field rows and their shipped default states: `First Name` `Last Name` `Email` (locked) · **Biography (off)** · **Headshot (off)** · Company Name (on) · Job Title (on) · Mobile Phone (on) · Home Phone (on) · `⊕ Add Question`. Bio and headshot being **off by default on the CFP form** is deliberate — they are collected later, in the portal, from accepted speakers only.
  - Field labels are **edited inline**: clicking the pencil turns the label into an editable rich-text cell with a mini toolbar, rather than opening a modal.
- **Portal forms are a separate builder** (`Portals → Forms`): 3 pages — *Form Set Up* (Internal Form Name · External Form Title · Page Heading) · *Form Questions* (Section Title · Description & Instructions · `+ Add Field`) · *Form Settings* (**Send confirmation email** → emails the contact **a PDF of their own responses**, with an editable body). Types: **Contacts / Groups / Sessions**. Pre-existing data **pre-fills** the form so the user reviews-and-updates rather than retypes. Results have their own **View Submissions** table with Columns/Filters/Sort, CSV/XLSX export, and **Download Forms** (PDF, only available if the confirmation email is enabled). `NEW` — this is the complete spec for TODO item [13].
  ...
  Frame-verified additions: the list page tabs are **All Forms (9) · People Forms (4) · Group
  Forms (3) · Session Forms (2)** with `Sort by: Most Results`, and each row reads
  *"3 submissions · Contact · Created by <name> on <date>"*. **Create Portal Form** is a small
  modal — *"Who do you want to collect information from?"* → the same three icon cards
  (Contacts / Groups / Sessions) → **Save & Build Form**. The builder's Form Questions step says,
  verbatim, **"Add questions you want your contacts to answer. Each question will update the
  relevant fields on the contact profile."** — i.e. **a portal form writes straight into the
  contact/session record**, it is not a standalone answer blob. That is the single most important
  design decision to copy: it is why pre-existing data pre-fills, and why "Review and update your
  contact information" works as a task. Adding a question opens a **Select fields** modal —
  `Filter fields…`, `Select All / Remove All`, an `All Fields | Global | Event` segmented control,
  a checkbox list of every standard and custom field with an ⓘ per row, a **Selected Fields**
  basket on the right, and `Cancel · Done · ⊕ Create New Field`.
...
4. **Home** shows the welcome message, **My Sessions**, **My Profile**, and the assigned Tasks / Forms / Files / Resources widgets. The portal nav pills in the frames are **Home · Sessions · Profile · Tasks · Files · Resources** — the last two appear only when that portal has files/wiki pages assigned, so our **Home · Submissions · Profile · Tasks** is the same set minus the two swyx struck. A task row in the portal carries a **status word** — the observed set is `Completed · Incomplete · **Denied**`, where *Denied* is a file request the organizer rejected and the speaker must re-upload. `NEW` — we have no denied state on the speaker side.
...
Ninety-three distinct findings are labelled `NEW` above. Grouped by area for the parity loop:
...
| **4** | **Public-visibility flags at both granularities.** Every accepted speaker and session of ours is public the moment the agenda is published. Sessionboard has a per-session-participant **eye toggle** (`is_public`, honoured by embeds *and* the API) **and** a standard **`Display Session` checkbox** on the session itself. | S1 | S | Together these *are* sbek **CNT-12** ("unapproved content excluded from public output") — and their answer is refreshingly simple: two booleans on the records, not an approval workflow. The embargoed-keynote case is universal. `submissionParticipants.isPublic` + `submissions.displayPublicly` + a filter in `publicData`/`apiV1` + an eye button in the drawer. |
