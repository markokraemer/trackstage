...
| 84 | Submission triggers a confirmation email referencing the submission. | SBEK CFP-08 | **COVERED** (fixed 2026-08-11) | `convex/submit.ts:submit` → `internal.comms.queueForPerson` (`templateKey: "confirmation"`) | Queued for the submitter **and** every speaker participant, with `submissionId` so `{{sessionTitle}}` resolves; gated on `participantConfig.sendConfirmationEmail`. Verified both ways in `scripts/verify-backend.mjs` ("Submission side-effects"). |
...
| 112 | Blind/anonymized review round hides author identity from reviewers. | SBEK ABS-07 | **COVERED** (fixed 2026-08-11) | `convex/review.ts:queue` (`anonymized`), `convex/evaluationsAdmin.ts:createPlan/updatePlan`, `src/components/evaluation/new-plan-dialog.tsx`, `src/routes/review/$token.tsx` | Enforced server-side: a blind plan never loads `submissionParticipants` at all, so no name/job title/company reaches the wire. `queue` returns `anonymized: true`; the review page swaps the speakers row for a "Blind review — score this on the abstract alone" note plus a badge; the plan dialog has a **Blind review** switch and the plan card a **Blind** badge. |
...
| 119 | Comments on an uploaded file with author + timestamp, cross-role visible. | SBEK CNT-05 | **DONE** (2026-08-11) | `convex/schema.ts` `uploadComments`, `convex/lib/uploadComments.ts`, `convex/tasksAdmin.ts` `listUploadComments/addUploadComment`, `convex/portal.ts` `uploadComments/addUploadComment`, `src/components/shared/file-comments.tsx` | One thread per file, oldest first, author label + role + timestamp captured at write time; organizer reads it on the submission drawer's Files tab, the speaker on their portal file rows. No email on a new comment (v1, as Sessionboard ships it). |
...
| 122 | Change/version history with attribution + timestamp; restore works. | SBEK CNT-11 | **COVERED (history) / DEFERRED (restore)** | `convex/schema.ts` `auditLog`, `convex/audit.ts` (`forEntity`, `feed`), `convex/lib/audit.ts`, submission drawer **History** tab, **Settings → Activity** | 2026-08-11: append-only audit log with attribution + timestamp on every high-value write (status changes, queue commits, form/agenda/speaker/settings edits) and on every agent write (MCP tool + key prefix, REST method + key prefix, Airtable pull, API-key lifecycle). RESTORE is deliberately not built — Marko/swyx judged full versioning overkill for v1 (HISTORY.md 61), so the rubric's restore half stays open by choice. |
...
| 21 | ~~#122~~, #119, #124, #129, #136 | ~~No content change-history~~ (DONE 2026-08-11 — audit log; restore deferred by decision), no file comments, no bulk ZIP, no speaker CSV import, no travel/logistics fields | S3 | L | sbek ~~CNT-11~~/CNT-05/CNT-14, SPK-03/SPK-15. Four 1–2-weight items left; only worth it after ranks 1–15. |
